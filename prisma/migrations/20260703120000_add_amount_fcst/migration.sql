IF COL_LENGTH('dbo.forecast_values', 'amountFcst') IS NULL
BEGIN
  ALTER TABLE [dbo].[forecast_values]
  ADD [amountFcst] DECIMAL(18, 4) NOT NULL
    CONSTRAINT [forecast_values_amountFcst_df] DEFAULT 0;
END;

IF COL_LENGTH('dbo.forecast_change_logs', 'oldAmountFcst') IS NULL
BEGIN
  ALTER TABLE [dbo].[forecast_change_logs]
  ADD [oldAmountFcst] DECIMAL(18, 4) NULL;
END;

IF COL_LENGTH('dbo.forecast_change_logs', 'newAmountFcst') IS NULL
BEGIN
  ALTER TABLE [dbo].[forecast_change_logs]
  ADD [newAmountFcst] DECIMAL(18, 4) NOT NULL
    CONSTRAINT [forecast_change_logs_newAmountFcst_df] DEFAULT 0;
END;
